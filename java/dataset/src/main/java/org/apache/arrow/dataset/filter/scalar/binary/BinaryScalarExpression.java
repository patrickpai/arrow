...
public class BinaryScalarExpression extends ScalarExpression {

    private final NullableVarBinaryHolder holder;
    private final BufferAllocator allocator;

    public BinaryScalarExpression(BufferAllocator allocator, VarBinaryHolder holder) {
        this(allocator, holder.isSet, holder.buffer, holder.start, holder.end);
    }

    public BinaryScalarExpression(BufferAllocator allocator, NullableVarBinaryHolder holder) {
        this(allocator, holder.isSet, holder.buffer, holder.start, holder.end);
    }

    private BinaryScalarExpression(BufferAllocator allocator, int isSet, ArrowBuf buffer, int start, int end) {
        NullableVarBinaryHolder clone = new NullableVarBinaryHolder();
        clone.isSet = isSet;
    ...
    public StructVector toVector(String vectorName, BufferAllocator allocator) {
        ...
        Util.addVarBinaryVectorAsChild(vector, "c1", holder);
    ...
    public Expression deepClone() {
        return new BinaryScalarExpression(allocator, holder);
    }
